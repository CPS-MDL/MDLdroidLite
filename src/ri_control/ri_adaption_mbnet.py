...
#################################
# 1
# growth_list = [(0,2),(2,2),(4,2),(6,2)]
def grow(model, growth_list, mode='rank_cosine', ff=1, inputs=None, target=None, features_output=None, test_loader=None, criterion=None):
...
# weights adaption
def weight_adaption(old_layer, incremental_num=1, generate_mode='rank_cosine', is_first=True, is_second=False, is_multi=False, rank_list=[]):
    if generate_mode == 'random':
        ...
        weight, bias = weight_adaption_rank_cosine(old_layer, incremental_num, is_first=is_first, is_second=is_second, rank_list=rank_list)
    else:
...
def conv_layer_grow(old_conv_layer, incremental_num, is_first=True, is_second=False, mode='rank', rank_list=[]):
    ...
    if mode != 'random':
        weights, bias = weight_adaption(old_conv_layer, incremental_num, generate_mode=mode, is_first=is_first, is_second=is_second, rank_list=rank_list)
        old_grad = old_conv_layer.weight.grad.data.cpu().numpy()

    # create new_conv_layer
    if is_first:

        new_conv_layer = torch.nn.Conv2d(in_channels=(old_conv_layer.in_channels + incremental_num) if old_conv_layer.groups > 1 else old_conv_layer.in_channels,
                                         ...
                                         groups=(old_conv_layer.groups + incremental_num) if old_conv_layer.groups > 1 else old_conv_layer.groups,
...
def fc_layer_grow(old_linear_layer, incremental_num, weights=None, is_first=True, mode='rank',
    ...
    if weights is None:
        ...
        weights, bias = weight_adaption(old_linear_layer, incremental_num, generate_mode=mode, is_first=is_first,
                                        is_multi=is_multi, rank_list=rank_list)
    else:
...
def grow_filters(model, layer_index, incremental_num, mode='rank_cosine'):
    global cosine_rank
    layers = find_modules(model)
    old_layer = layers[layer_index]
    old_layer2 = None
    ...
    # first
    new_layer = conv_layer_grow(old_layer, incremental_num, is_first=True, mode=mode)
    rank_list = cosine_rank
    ...
    if layer_index < len(layers) - 2:
        ...
        # second
        # old_layer2_1 = conv_layer_grow(old_layer2, incremental_num, is_first=False, mode=mode, rank_list=rank_list)

        new_layer2 = conv_layer_grow(old_layer2, incremental_num, is_first=True, is_second=True, mode=mode, rank_list=rank_list)

        # third
        new_layer3 = conv_layer_grow(old_layer3, incremental_num, is_first=False, mode=mode, rank_list=rank_list)
        ...
        # second fc
        old_layer2 = layers[layer_index + 1]
        new_layer2 = fc_layer_grow(old_layer2, incremental_num, is_first=False, mode=mode, rank_list=rank_list)
...
def weight_adaption_rank_cosine(old_layer, incremental_num, is_first=True, asc=True, is_second=False, rank_list=[]):
    global flatten_factor
    old_weights = old_layer.weight.data.cpu().numpy()
    new_bias = None
    cosine_array = []
    ...
    if isinstance(old_layer, torch.nn.Conv2d):
        ...
        if is_first:
            ...
            weights = np.zeros((old_c_out, incremental_num, k_h, k_w))
            for order in range(incremental_num):
                weights[:, order, :, :] = old_weights[:, rank_list[order], :, :]
