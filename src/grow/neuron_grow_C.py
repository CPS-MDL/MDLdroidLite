...
from grow.gate_hook import remove_hook, remove_gate_max
...
def train(train_loader, model, to_log, to_test, test_loader=None, grow_interval=1, grow_con_interval=300000000,
    ...
    for i_batch, (inputs, target) in enumerate(train_loader):
        ...
        if (epoch - 1) in grow_epoch and first_time:
            ...
            #                             print_weights=True)
            growth_list = [(0, 2), (1, 3), (2, 10)]
            model = ri_adaption.grow(model, growth_list, inputs, target, criterion, testloader)
            # scale_list = ri_adaption.get_scale()

            # print('scale list: {}'.format(scale_list))
            old_size = ri_adaption.get_old_size()
            existing_index = old_size
            new_size = ri_adaption.get_new_size()
            ...
            register_acv_hook(model)
            # ri_gate.register_hook(model, module_names, old_size, new_size, scale_list=scale_list)
            # make grad avaliable
            features_output, output = model(inputs)  # save acv after grow
            loss = criterion(output, target)
            loss.backward()
            # optimizer = ri_adaption.optimizer_adapt()
...
if __name__ == '__main__':
    ...
    for mode in loop_list:
        ...
        trainloader, testloader = generate_data_loader(batch_size=param['batch_size'], dataset=dataset,
                                                       is_main=False)
        ri_adaption = RIAdaption(mode='ours')
