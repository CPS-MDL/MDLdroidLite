...
def weight_adaption_copy_one(old_layer, incremental_num, is_first=True, is_multi=False, multi=16):
    ...
    if isinstance(old_layer, torch.nn.Conv2d):
        ...
        if is_first:
            ...
            random = random_num
            for i in range(incremental_num):
                weights[:, i, :, :] = old_weights[:, random, :, :] / numerator
            size_needed = np.random.uniform(np.min(old_weights) / 10, np.max(old_weights) / 10,
                                            (old_c_out, incremental_num, k_h, k_w))
...
def weight_adaption_rank_connect(old_layer, incremental_num, is_first=True, is_mutli=False, m_seed=16):
    global divide_num
...
def weight_adaption_rank_baseline(old_layer, incremental_num, is_first=True, is_mutli=False):
    global divide_num
    global importance_rank
    old_weights = old_layer.weight.data.cpu().numpy()
    new_bias_needed = None
...
def weight_adaption_rank_connect_noscale(old_layer, incremental_num, is_first=True, is_mutli=False):
    old_weights = old_layer.weight.data.cpu().numpy()
    new_bias_needed = None
    global divide_num
    numerator = 1
...
def weight_adaption_rank_group(old_layer, incremental_num, is_first=True, multi=16, is_mutli=False):
    old_weights = old_layer.weight.data.cpu().numpy()
    new_bias_needed = None
    cosine_array = []
    global cosine_rank
...
def weight_adaption(old_layer, incremental_num, generate_mode='avg', is_first=True, is_multi=False):
    ...
    if generate_mode == 'rank_baseline':
    ...
    if generate_mode == 'rankconnect':
        weight, bias = weight_adaption_rank_connect(old_layer, incremental_num, is_first=is_first,
                                                    is_mutli=is_multi)
    if generate_mode == 'copy_one':
        weight, bias = weight_adaption_rank_connect(old_layer, incremental_num, is_first=is_first,
                                                    is_mutli=is_multi)
    if generate_mode == 'copy_n':
...
def weights_dived_first(old_weights, incremental_num, multi=1, twoD=False, mode='rankconnect'):
    global cosine_rank
    global random_num
    global importance_rank
    global divide_seed
    ...
    for i in range(incremental_num):
        if mode == 'rankgroup':
            ...
            order = random_num[i]
        elif mode == 'rankconnect' or mode == 'rank_baseline':
            order = importance_rank.index(i)
        start = order * multi
        end = start + multi
        if twoD:
            old_weights[start:end] = old_weights[start:end] / (seed * multi)
...
def conv_layer_grow(old_conv_layer, incremental_num, is_first=True, mode='rank', rank=None):
    ...
    if mode == 'rank-baseline':
        old_weights = weights_dived_first(old_weights, incremental_num, twoD=False, mode=mode)
...
def fc_layer_grow(old_linear_layer, incremental_num, weights=None, is_first=True, mode='rank', rank=None,
    ...
    if mode == 'rank_baseline':
        old_weights = weights_dived_first(old_weights, incremental_num, twoD=True, mode=mode)
