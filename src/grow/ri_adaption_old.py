...
from grow.growth_utils import put_to_tensor, get_filter_rank, get_rank, target_std_scale, put_to_ndarray
...
class RIAdaption:
    def __init__(self, model, optimizer, mode, all_scale=True, new_bias=True):
        self.old_size = []
        self.new_size = []
    ...
    def _structure_model(self):
        ...
        for index_str, module in self.model.features._modules.items():
            if isinstance(module, torch.nn.Conv2d):
                struct.append((self.model.features, int(index_str)))

        for index_str, module in self.model.classifier._modules.items():
            if isinstance(module, torch.nn.Linear):
                struct.append((self.model.classifier, int(index_str)))
        self.struct = struct
    ...
    def grow(self, growth_list, inputs=None, target=None, criterion=None, testloader=None):
        # reset scale_list
        self.scale_list = []

        # rest old_size
        self.old_size = model_size(self.modules[:-1])
        test(self.model, test_loader=testloader, criterion=criterion, to_log=None, is_two_out=False)
        ...
        for i, grow_list in enumerate(growth_list):
            ...
            if inputs is not None:
                ...
                loss.backward()
            self._generator(first, second, num_growth)
            # test
            test(self.model, test_loader=testloader, criterion=criterion, to_log=None, is_two_out=False)
    ...
    def _generator(self, first=0, second=1, num_growth=0):
        if num_growth == 0:
            if self.mode == 'ours':
    ...
    def optimizer_adapt(self):
        lr = self.optimizer.defaults['lr']
        if self.optimizer_mode == 'AdamW':
            new_optimizer = AdamW(self.model.parameters(), lr=lr)
            new_optimizer.vs = self.optimizer.vs
            new_optimizer.ms = self.optimizer.ms
            new_optimizer.gs = self.optimizer.gs
            new_optimizer.grads = self.optimizer.grads
            new_optimizer.vg = self.optimizer.vg

        for current_group, new_group in zip(self.optimizer.param_groups, new_optimizer.param_groups):
            layer_index = 0
            for current_p, new_p in zip(current_group['params'], new_group['params']):
                ...
                current_state = self.optimizer.state[current_p]
    ...
    def _adaption(self, old, new, is_first, is_flatten, num_growth):
        ...
        old_c = old.weight.data.shape[dim]
        new_c = new.weight.data.shape[dim]
        ...
        if self.mode == 'ours':
            ...
            std = np.std(old_tensor.cpu().numpy())
            scale = target / std
        ...
        # new weight and bias
        new_weight, new_bias = self._gen_new_weights(old, num_growth, project_index, is_first, is_flatten)
    ...
    def _gen_new_weights(self, old, num_growth, project_list=None, is_first=True, is_flatten=False, min_value=-0.1,
                         max_value=0.1):
        new_bias = None
        ...
        t_noise = torch.zeros_like(old_weight.narrow(dim, -num_growth * length, num_growth * length)).uniform_(min_value, max_value)
        noise_shape = t_noise.cpu().numpy().shape
        noise = torch.from_numpy(np.random.uniform(min_value, max_value, noise_shape).astype(np.float32)).to(device)
