...
def train(train_loader, model, to_log, to_test, test_loader=None, grow_interval=1, grow_con_interval=300000000,
    ...
    for i_batch, (inputs, target) in enumerate(train_loader):
        ...
        if is_gate and existing_index and not first_time:
            if i_batch == 1:
                pass
        ...
        if (epoch) in grow_epoch and first_time and i_batch == 599:
            ...
            # guarantee gradient
            features_output, tem_output = model(inputs)  # save acv after grow
            loss = criterion(tem_output, target)
            loss.backward()
