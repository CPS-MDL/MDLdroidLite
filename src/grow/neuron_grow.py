...
from grow.gate_hook import remove_hook, register_hook_delete, remove_gate_max
...
from utils import write_log, AverageMeter, accuracy, test, save_model, dir_path, count_parameters, calculate_l1, \
    ...
    fill_zeros_to_diff, target_std_scale_calculator
...
def weight_adaption_rank_cosine_2(old_layer, incremental_num, is_first=True, multi=16, is_multi=False, asc=True):
    old_weights = old_layer.weight.data.cpu().numpy()
    new_bias = None
    cosine_array = []
    global cosine_rank
...
def weight_adaption_rank_cosine_error(old_layer, incremental_num, is_first=True, multi=16, is_multi=False, asc=True):
    old_weights = old_layer.weight.data.cpu().numpy()
    old_bias = old_layer.bias.data.cpu().numpy()
    new_bias = None
    cosine_array = []
...
def weight_adaption_rank_cumulative_cosine_old(old_layer, incremental_num, is_first=True, multi=16, is_multi=False,
                                               ascending=False):
    old_weights = old_layer.weight.data.cpu().numpy()
    new_bias_needed = None
    cosine_array = []
    ...
    for i_batch, (inputs, target) in enumerate(train_loader):
        ...
        if is_gate and not first_time:
            ...
            if i_batch < gate_step:
                ri_gate.step(recorder.get_pre_score(), recorder.get_score(), recorder.get_pre_weight(),
                             recorder.get_weight(), existing_index, step=i_batch)
                ri_gate.record_gate(epoch, i_batch)

            if i_batch == gate_step - 1:
                ri_gate.reset_grad_gate()
                ri_gate.reset()
                # print("Test before putting gate back:")
                # test(model, test_loader=test_loader, criterion=criterion, to_log=to_test,
                #      is_two_out=True)
                # ri_gate.put_gate_back(model)
                # print("Test after putting gate back:")
                # test(model, test_loader=test_loader, criterion=criterion, to_log=to_test,
                #      is_two_out=True)

        # gradient decay
        if is_gate and existing_index:
            ri_gate.gradient_decay(model, existing_index)
            # ri_gate.gradient_decay_lambda(model)
            # gradient_lambda_decay(model)
            # gradient_lambda_decay_select(model, lambda_list)

        # if (epoch - 1) not in grow_epoch and existing_index:
        #     ri_gate.step_without_grow(splite_index=existing_index, step=i_batch)
        ...
        if (epoch - 1) in grow_epoch and first_time:
            ...
            for i, layer_index in enumerate(layer_index_list):
                ...
                #
                # calculate_score(model, inputs, target, epoch=epoch, channel=channel_after, print_weights=False)
            ...
            # weight decay
            if is_gate and existing_index:
                scale_list = weight_decay(model, existing_index, layer_index_list)
            ...
            if is_gate:
                ri_gate.register_hook(model, module_names, old_size[:3], new_size[:3], scale_list=scale_list)
        ...
        #             # if i_batch == 49:
        #             #     print('', end='')
        #             ri_gate.step(recorder.get_pre_score(), recorder.get_score(), existing_index, step=i_batch,
        ...
        #     ri_gate.step(recorder.get_pre_score(), recorder.get_score(), existing_index, step=i_batch,
        #                  total_step=gate_step, is_print=True)
...
def train_test(model, trainloader, testloader, learning_rate, epoch, dic_path, str_channel):
    ...
    for e in range(epoch):
        ...
        # print(hex(id(model)))
        # print(hex(id(model.features)))
        # print(hex(id(model.classifier)))
        # print(hex(id(model.features._modules['0'])))
        # print(hex(id(model.features._modules['3'])))
        # print(hex(id(model.classifier._modules['0'])))
        # print(hex(id(model.classifier._modules['2'])))

        # print(model.features._modules['0'].weight.data)
        # print(model.features._modules['0'].bias.data)
        # print(model.features._modules['3'].weight.data)
        # print(model.features._modules['3'].bias.data)
        # print(model.classifier._modules['0'].weight.data)
        # print(model.classifier._modules['0'].bias.data)
        # print(model.classifier._modules['2'].weight.data)
        # print(model.classifier._modules['2'].bias.data)
...
if __name__ == '__main__':
    ...
    std_para_list = [
        # {'out1': 2, 'out2': 5, 'fc1': 20},
        {'out1': 10, 'out2': 25, 'fc1': 250},
        {'out1': 14, 'out2': 35, 'fc1': 350},
        {'out1': 18, 'out2': 45, 'fc1': 450},
